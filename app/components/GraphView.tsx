...
export default function GraphView({
  ...
  useEffect(() => {
    ...
    return () => {
      if (bodyOverflowRef.current) {
        document.body.style.setProperty("overflow", bodyOverflowRef.current);
      } else {
  ...
  return (
    <div
      ...
      aria-label={isFullscreen ? "그래프 전체화면 보기" : undefined}
      className={`flex flex-col overflow-hidden lg:flex-row ${
        isFullscreen
          ? "fixed inset-0 z-50 h-dvh rounded-none border-0"
          : "h-full"
      }`}
      style={{ backgroundColor: SCREEN_GRAPH_PAPER, color: SCREEN_INK, ...BODY_FONT }}
    >
      <aside
        className={`max-h-[36vh] w-full shrink-0 overflow-y-auto border-b px-4 py-5 lg:max-h-none lg:border-b-0 lg:border-r lg:px-5 lg:py-6 ${
          isFullscreen ? "lg:w-[320px]" : "lg:w-[300px]"
        }`}
      ...
      <div className="relative min-h-0 min-w-0 flex-1 overflow-hidden" style={{ backgroundColor: SCREEN_CANVAS_PAPER }}>
        <div className="absolute right-3 top-3 z-20 flex max-w-[calc(100%-24px)] flex-wrap items-center justify-end gap-2 lg:right-6 lg:top-6 lg:max-w-none">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-[12px] border px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.16em]"
            style={{
              borderColor: "var(--border-strong)",
              backgroundColor: "var(--bg)",
              color: SCREEN_TEXT_SUBTLE,
          ...
          >
            <svg aria-hidden="true" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              {isFullscreen ? (
                <>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 4v5H4" />
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 4v5h5" />
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 20v-5H4" />
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 20v-5h5" />
                </>
              ) : (
                <>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4 9V4h5" />
            ...
            <span className="hidden sm:inline">{isFullscreen ? "닫기" : "전체화면"}</span>
          </button>
          <div
